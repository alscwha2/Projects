...
int main(int argc, char *argv[]) {
	...
	char **lines = convert_string_to_array_of_lines(get_file_contents(file_name));
	if(argc == GREP || argc == GREPWC) {
		lines = filter_lines(lines, argv[2]);
	}
	if (argc == WC || argc == GREPWC) {
		print_count(count_words(convert_nonABC(find_words(to_lower_case(lines)))));
		return EXIT_SUCCESS;
	}
	if (argc == GREP) {
		print_lines(lines);
...
char **filter_lines(char **lines, char *grep_string) {
	...
	for (int i = 1; i <= number_of_lines; i++) {
		if(!(strstr(lines[i], grep_string) == NULL)) {
			new_lines[new_number_of_lines + 1] = lines[i];
			new_number_of_lines++;
		} else {
			free(lines[i]);
		}
	...
	new_lines = realloc(new_lines, (new_number_of_lines + 1) * sizeof(char *));
	free(lines);
	return new_lines;
}
